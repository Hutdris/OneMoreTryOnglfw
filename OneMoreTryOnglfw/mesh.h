...
public:
	...
	/*  Functions  */
	// Constructor
	Mesh(string stl_path)
	{
		this->load_stl_ascii(stl_path);
		// Now that we have all the required data, set the vertex buffers and its attribute pointers.
		this->setupMesh();
	}
		void load_stl_ascii(string stl_path) {
			std::ifstream stlFile;
			stlFile.open(stl_path);
			...
			while (1)
			{
				...
				for (int i = 0; i <= 2; ++i) // read the three vertices of a face
				{
					...
					vertexTmp.TexCoords = glm::vec2(0.0f, 0.0f);
					this->vertices.push_back(vertexTmp);
				}
